        ...
        <h3 className="font-serif text-md mb-3">
          {portfolio.projectUrl ? (
            <a
              ...
              className="group inline-flex items-center gap-2 hover:text-link-hover transition-colors duration-300"
            >
              {portfolio.title}
              <ArrowUpRight
                size={16}
        ...
        {portfolio.technologies && (
          <div className="flex gap-2 mb-4 flex-wrap">
            {portfolio.technologies.map((tech, index) => (
              <span
                key={index}
                className="text-xs text-muted px-2 py-1 bg-tag-bg rounded-full"
              >
                {tech}
              </span>
            ))}
